...
class Product(models.Model):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="mes_produits"
    )
    nom = models.CharField(max_length=200)
...
class Commande(models.Model):
    ...
    def __str__(self):
        ...
        date_str = (
            self.date_commande.strftime("%d/%m/%Y %H:%M") if self.date_commande else ""
        )
...
class Discussion(models.Model):
    acheteur = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="discussions_acheteur",
    )
    vendeur = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="discussions_vendeur",
    )
...
class SMS(models.Model):
    ...
    def __str__(self):
        return f"SMS à {self.numero_destinataire} - {self.date_envoi.strftime('%d/%m/%Y %H:%M')} - {self.get_statut_display()}"
